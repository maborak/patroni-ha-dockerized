...
4. [Remote PITR](#remote-pitr)
5. [Scenarios](#scenarios)
6. [WAL Methods](#wal-methods)
...
```bash
# No arguments → step-by-step wizard:
#   select backup → target node → WAL method → target time → review → confirm
make pitr
```
...
Use `--target ssh://user@host[:port]/absolute/path` to ship the recovered `PGDATA` to a **non-cluster host** over SSH + rsync instead of applying it to a local node:
...
Behavior:
- Ships the recovered data directory via SSH + rsync (compressed, resumable), then prints next steps for starting PostgreSQL manually on the remote host.
- **Incompatible with `--restore` / `--auto-start`** (the remote host is not Patroni-orchestrated).
- Refuses shallow paths such as `/var`, `/tmp`, or `/root` — use a specific data-directory path.

`PITR_ALLOW_PIPED=1` is a testing escape hatch that allows the interactive wizard to run with piped (non-tty) stdin; it is normally reserved for the smoke tests.
...
# Data dir is /var/lib/postgresql/<PG_VERSION>/<CLUSTER_NAME>/ (e.g. /var/lib/postgresql/15/patroni1)
docker exec db2 cat /var/lib/postgresql/15/$PATRONI_CLUSTER_NAME/postgresql.auto.conf | grep recovery_target
