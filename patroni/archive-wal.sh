...
#
# Each node ships its own WAL to its own directory on the Barman host
# (barman.conf defines one server per node), so no leader gate is needed.
...
BARMAN_TARGET="barman@barman:/data/pg-backup/$(hostname)/incoming/${WAL_NAME}"
