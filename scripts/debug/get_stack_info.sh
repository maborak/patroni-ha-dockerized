...
# Check containers — use integer-indexed arrays for macOS/Bash 3.2 compatibility
NODE_RUNNING=()
NODE_ROLE=()
...
for db in "${DB_NODES[@]}"; do
    _num=$(get_node_num "$db")
    NODE_ROLE[$_num]="unknown"
    print_progress "Checking $db Patroni role" "$MAGENTA"
    if [ "${NODE_RUNNING[$_num]}" = true ]; then
        NODE_ROLE[$_num]=$(get_patroni_role "$db" "$INTERNAL_API_PORT" 2>/dev/null || echo "unknown")
        if [ "${NODE_ROLE[$_num]}" != "unknown" ]; then
            print_status "$db role: ${NODE_ROLE[$_num]}" "ok"
        else
...
EOF
    ...
    for db in "${DB_NODES[@]}"; do
        ...
        echo "  Status: $([ "${NODE_RUNNING[$_num]}" = true ] && echo "Running" || echo "Not Running")"
        echo ""
    done
