...
for db in $DATABASES; do
    ...
    if [ "$TOTAL_ROWS" = "0" ] || [ -z "$TOTAL_ROWS" ]; then
        ...
        TABLE_COUNT_CHECK=$(run_query "$db" "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog', 'information_schema') AND table_type = 'BASE TABLE';" || echo "0")
        if [ -n "$TABLE_COUNT_CHECK" ] && [ "$TABLE_COUNT_CHECK" -gt 0 ] && [ "$TABLE_COUNT_CHECK" -lt 100 ]; then
            TOTAL_ROWS=0
            TABLES=$(docker exec "$LEADER_NODE" psql -U postgres -d "$db" -p 5431 -h localhost -t -A -c "SELECT table_schema||'.'||table_name FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog', 'information_schema') AND table_type = 'BASE TABLE' ORDER BY table_schema, table_name;" 2>/dev/null || true)
