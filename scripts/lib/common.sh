...
# Node configuration
# ============================================================================
# Total members = replicas + 1 leader (Patroni elects the leader from members)
...
# Get the PostgreSQL major version of the running cluster (e.g. "15", "16").
# Precedence: POSTGRES_VERSION env > probe a running db container > fallback "15".
# Result is cached in _PATRONI_PG_VERSION_CACHE for the current shell.
get_patroni_pg_version() {
    if [ -n "${_PATRONI_PG_VERSION_CACHE:-}" ]; then
        echo "$_PATRONI_PG_VERSION_CACHE"
        return 0
    fi
    if [ -n "${POSTGRES_VERSION:-}" ]; then
        _PATRONI_PG_VERSION_CACHE="$POSTGRES_VERSION"
        echo "$_PATRONI_PG_VERSION_CACHE"
        return 0
    fi
    # Probe any running db container — pick the highest /var/lib/postgresql/<N> dir
    local v=""
    for n in $(get_db_nodes); do
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^${n}$"; then
            v=$(docker exec "$n" ls /var/lib/postgresql/ 2>/dev/null | grep -E '^[0-9]+$' | sort -n | tail -1)
            [ -n "$v" ] && break
        fi
    done
    _PATRONI_PG_VERSION_CACHE="${v:-15}"
    echo "$_PATRONI_PG_VERSION_CACHE"
}

# Get the Patroni data directory (same scope for all nodes)
get_patroni_data_dir() {
    echo "/var/lib/postgresql/$(get_patroni_pg_version)/${PATRONI_CLUSTER_NAME}"
}
