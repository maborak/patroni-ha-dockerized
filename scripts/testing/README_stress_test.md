...
Requires `psycopg2-binary`, already declared in the repo-root `requirements.txt`:
...
See also: the sibling `smoke_test_wizard.sh` / `smoke_test_pitr.sh` power `make smoke-test` (sandboxed, no Docker needed).
