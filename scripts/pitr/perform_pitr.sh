...
# Parse a --target value of the form ssh://user@host[:port]/abs/path or ssh:user@host[:port]/abs/path.
# Populates REMOTE_PITR_* globals. Returns non-zero with a printed error on bad input.
parse_remote_pitr_target() {
    local raw="$1"
    REMOTE_PITR_RAW="$raw"
    local rest="${raw#ssh://}"
    rest="${rest#ssh:}"
    if [[ "$rest" != *@* ]]; then
        echo -e "${RED}✗ Invalid SSH target (missing 'user@'): $raw${NC}" >&2
        return 1
    fi
    REMOTE_PITR_USER="${rest%%@*}"
    local hostpart="${rest#*@}"
    if [[ "$hostpart" != */* ]]; then
        echo -e "${RED}✗ Invalid SSH target (missing '/path'): $raw${NC}" >&2
        echo -e "${YELLOW}  Expected: ssh://user@host[:port]/absolute/path${NC}" >&2
        return 1
    fi
    local hostport="${hostpart%%/*}"
    REMOTE_PITR_PATH="/${hostpart#*/}"
    REMOTE_PITR_PATH="${REMOTE_PITR_PATH%/}"   # strip trailing slash for consistency
    if [[ "$hostport" == *:* ]]; then
        REMOTE_PITR_HOST="${hostport%:*}"
        REMOTE_PITR_PORT="${hostport##*:}"
    else
        REMOTE_PITR_HOST="$hostport"
        REMOTE_PITR_PORT=22
    fi
    if ! [[ "$REMOTE_PITR_PORT" =~ ^[0-9]+$ ]]; then
        echo -e "${RED}✗ Invalid SSH port: '$REMOTE_PITR_PORT'${NC}" >&2
        return 1
    fi
    if [ -z "$REMOTE_PITR_USER" ] || [ -z "$REMOTE_PITR_HOST" ] || [ -z "$REMOTE_PITR_PATH" ]; then
        echo -e "${RED}✗ Invalid SSH target — missing user/host/path${NC}" >&2
        return 1
    fi
    # A second '@' in the URI is almost always a typo (ssh://root@root@host/path
    # → we'd parse user=root, host='root@host', which fails opaquely at SSH time).
    if [[ "$REMOTE_PITR_HOST" == *@* ]]; then
        echo -e "${RED}✗ Host contains '@' character (likely a typo: '$REMOTE_PITR_HOST').${NC}" >&2
        echo -e "${YELLOW}  Use one user@ only: ssh://user@host[:port]/path${NC}" >&2
        return 1
    fi
    # Defensive: refuse obviously-too-shallow paths to limit blast radius if user typo'd.
    case "$REMOTE_PITR_PATH" in
        /|/var|/var/lib|/var/lib/postgresql|/home|/root|/tmp|/etc|/usr|/usr/local)
            echo -e "${RED}✗ Refusing to use shallow path '$REMOTE_PITR_PATH' as PITR target — too risky${NC}" >&2
            echo -e "${YELLOW}  Use a specific data-directory path, e.g. /var/lib/postgresql/15/main${NC}" >&2
            return 1
            ;;
    esac
    return 0
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --target|--target=*)
            # Support both space form (--target X) and equals form (--target=X)
            if [[ "$1" == --target=* ]]; then
                _tgt="${1#--target=}"
                shift
            else
            ...
            # Detect ssh:// or ssh: prefix → remote PITR mode
            if [[ "$_tgt" =~ ^ssh(:/{0,2}) ]]; then
                REMOTE_PITR_MODE=true
                parse_remote_pitr_target "$_tgt" || exit 1
                TARGET_NODE="ssh:${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_PITR_PORT}${REMOTE_PITR_PATH}"
            else
                AUTO_APPLY=true
                TARGET_NODE="$_tgt"
            fi
            ;;
        --server|--server=*)
            if [[ "$1" == --server=* ]]; then BACKUP_SERVER="${1#--server=}"; shift
            else BACKUP_SERVER="$2"; shift 2
            fi
            ;;
        --restore)
            START_RESTORE=true
        ...
        --auto-start)
            ...
            ;;
        *)
            # Reject misspelled / unparsed flags being silently consumed as positionals.
            if [[ "$1" == --* ]]; then
                echo -e "${RED}✗ Unknown flag: $1${NC}" >&2
                echo -e "${YELLOW}  Valid flags: --server, --target, --restore, --auto-start, --wal-method${NC}" >&2
                echo -e "${YELLOW}  Both space form (--server db1) and equals form (--server=db1) are accepted.${NC}" >&2
                exit 1
            fi
            if [ -z "$BACKUP_ID" ]; then
                BACKUP_ID="$1"
...
if [ -z "$BACKUP_ID" ] && [ -z "$TARGET_TIME" ] && [ $# -eq 0 ]; then
    # Check if running in interactive terminal (PITR_ALLOW_PIPED=1 opts in for tests)
    if [ ! -t 0 ] && [ "${PITR_ALLOW_PIPED:-0}" != "1" ]; then
            ...
            if [ -n "$BACKUPS" ]; then
                ...
                while IFS= read -r line || [ -n "$line" ]; do
                    if [ -n "$line" ]; then
                        ...
                        # NOTE: use assignment form, not ((var++)). With set -e at the
                        # top of this script, ((BACKUP_COUNT++)) returns the OLD value
                        # (0 on first call) which bash treats as false → script silently
                        # exits after "Loading backups..." even though everything worked.
    ...
    if [ -z "$BACKUP_ID" ] || [ -z "$TARGET_TIME" ]; then
        echo -e "${RED}Usage:${NC} $0 <backup-id> <target-time> [--server <server>] [--target <node|ssh://...>] [--restore] [--wal-method <method>]"
        echo ""
        echo "Examples:"
        echo "  # Stage only (no application)"
        echo "  $0 20260104T153446 '2026-01-04 15:45:00'"
        echo ""
        echo "  # Apply to a local cluster node (Patroni-orchestrated)"
        echo "  $0 20260104T153446 '2026-01-04 15:45:00' --server db2 --target db1"
        echo "  $0 20260104T153446 latest --server db2 --target db1 --restore --auto-start"
        echo ""
        echo "  # Ship recovered data to a REMOTE host via ssh+rsync (manual finish)"
        echo "  $0 20260104T153446 latest --target=ssh://root@1.2.3.4:2323/var/lib/postgresql/15/main"
        echo "  $0 20260104T153446 '2026-01-04 15:45:00' --target ssh://postgres@db.example.com/pgdata"
        echo ""
        echo "Options:"
        ...
        echo "  --target <node>    Local cluster node (db1, db2, db3, db4) — Patroni-orchestrated."
        echo "                     This will: stop Patroni → backup data dir → rsync recovered files"
        echo "                                → set permissions → optionally start + monitor."
        echo "  --target ssh://user@host[:port]/path"
        echo "                     REMOTE PITR: ship recovered \$PGDATA to a non-cluster host via"
        echo "                     ssh+rsync, then print next-steps for the user to start PG manually."
        echo "                     Incompatible with --restore / --auto-start (manual finish only)."
        echo "  --restore          Start PostgreSQL recovery automatically (only with --target dbN)"
...
# Remote PITR (--target=ssh://...) is incompatible with local-cluster flags.
# The remote host's PostgreSQL must be stopped/started manually — we don't
# orchestrate Patroni or systemd on a box we don't own.
if [ "$REMOTE_PITR_MODE" = true ]; then
    if [ "$START_RESTORE" = true ] || [ "$AUTO_START" = true ]; then
        echo -e "${RED}✗ --restore / --auto-start cannot be combined with a remote ssh:// target.${NC}" >&2
        echo -e "${YELLOW}  Remote PITR ships files only; PostgreSQL must be started manually on the remote host.${NC}" >&2
        exit 1
    fi
fi

# Default target node if not specified (LOCAL mode only — REMOTE mode requires explicit target)
...
cleanup_staging_on_failure() {
    local ec=$?
    [ "$ec" -eq 0 ] && return 0
    if [ "${KEEP_TMP_ON_FAILURE:-0}" = "1" ]; then
        echo "" >&2
    ...
    local cleaned=0
    if [ -n "${HOST_RECOVERY_DIR:-}" ] && [ -d "$HOST_RECOVERY_DIR" ]; then
        echo "" >&2
        echo -e "${YELLOW}⚠ Cleaning up host staging: $HOST_RECOVERY_DIR${NC}" >&2
...
trap cleanup_staging_on_failure EXIT
...
# Step 6: Verify recovery files; optionally copy from container to host.
#
...
if docker exec backup test -d "$CONTAINER_RECOVERY_DIR" 2>/dev/null && [ "$(docker exec backup ls -A $CONTAINER_RECOVERY_DIR 2>/dev/null | wc -l)" -gt 0 ]; then
    ...
    if [ "$REMOTE_PITR_MODE" = true ]; then
        ...
        # Stage-only mode — copy to host /tmp so the user can inspect.
        HOST_RECOVERY_DIR="/tmp/pitr_recovery_host_$(date +%Y%m%d_%H%M%S)"
        echo -e "${CYAN}Copying recovery files from container to host (stage-only mode)...${NC}"
...
#
# Phase 2 — RSYNC HOST → REMOTE (resumable, verified):
#     rsync --partial-dir=.partial --append-verify --info=progress2 /tmp/pitr_<id>.tar.zst remote:/tmp/
#     - If wire drops at 14 GB, next run rsyncs only the tail
#     - --append-verify recomputes checksums on the resumed bytes
#     - Multi-target ready: same file can be rsync'd to N remotes
#
# Phase 3 — EXTRACT ON REMOTE (local I/O only):
#     ssh remote "zstd -dc /tmp/pitr_<id>.tar.zst | tar xpf - -C $PATH"
#     - No network in this phase; decompression and unpack are local on the remote
#     - Cleaned-up on success; kept on failure (so you can retry without re-shipping)
#
# Cleanup policy:
#   SUCCESS  → delete /tmp/pitr_<id>.tar.zst on host, delete container recovery dir
#   FAILURE  → keep BOTH (so a retry can resume from the partial archive)
if [ "$REMOTE_PITR_MODE" = true ]; then
    echo -e "${BLUE}${BOLD}========================================${NC}"
    echo -e "${BLUE}${BOLD}  Remote PITR Ship (3-phase staged: compress → rsync → extract)${NC}"
    echo -e "${BLUE}${BOLD}========================================${NC}"
    echo ""
    echo -e "${CYAN}Target:${NC} ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_PITR_PORT}${REMOTE_PITR_PATH}"
    echo ""

    # Disable the global failure-cleanup trap for the remote-PITR flow.
    # Rationale: staged transfers need partial state to survive failures (so a
    # retry can resume from where it broke). The success path below handles
    # cleanup explicitly.
    trap - EXIT
    ...
    if ! command -v rsync >/dev/null 2>&1; then
        echo -e "${RED}✗ rsync is required on the host for Phase 2 (resumable transfer). Install: brew install rsync / apt install rsync${NC}" >&2
        exit 1
    fi

    SSH_FAST_OPTS="-p ${REMOTE_PITR_PORT} -o StrictHostKeyChecking=accept-new -o Compression=no -o Ciphers=aes128-gcm@openssh.com,chacha20-poly1305@openssh.com"

    # --- Pre-flight: SSH + tool discovery ---
    echo -e "${YELLOW}[7.1/4] Pre-flight (SSH + tooling)...${NC}"
    if ! ssh -o ConnectTimeout=10 -o BatchMode=yes $SSH_FAST_OPTS \
        "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}" "echo ok" >/dev/null 2>&1
    then
        echo -e "${RED}✗ Cannot SSH to ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_PITR_PORT}${NC}" >&2
        echo -e "${YELLOW}  Test: ssh $SSH_FAST_OPTS ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST} 'echo ok'${NC}" >&2
        exit 1
    fi

    # Probe REMOTE in one round-trip for the tools we may need
    REMOTE_TOOLS=$(ssh -o BatchMode=yes $SSH_FAST_OPTS \
        "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}" \
        'for t in tar rsync zstd pigz gunzip sha256sum; do command -v $t >/dev/null 2>&1 && echo "$t"; done' 2>/dev/null || true)
    if ! echo "$REMOTE_TOOLS" | grep -qx 'tar'; then
        echo -e "${RED}✗ tar is required on the remote (for Phase 3 extract).${NC}" >&2
        exit 1
    fi
    if ! echo "$REMOTE_TOOLS" | grep -qx 'rsync'; then
        echo -e "${RED}✗ rsync is required on the remote (for Phase 2 resumable transfer).${NC}" >&2
        echo -e "${YELLOW}  Install: apt install rsync${NC}" >&2
        exit 1
    fi
    ...
    if [ "${TAR_NO_COMPRESS:-0}" != "1" ]; then
        if echo "$BARMAN_TOOLS" | grep -qx 'zstd' && echo "$REMOTE_TOOLS" | grep -qx 'zstd'; then
            COMP_NAME="zstd" ; COMP_EXT=".zst" ; CONT_COMP_CMD="zstd -1 -T0"  ; REMOTE_DECOMP_CMD="zstd -d"
        elif echo "$BARMAN_TOOLS" | grep -qx 'pigz' && echo "$REMOTE_TOOLS" | grep -qx 'pigz'; then
            COMP_NAME="pigz" ; COMP_EXT=".gz"  ; CONT_COMP_CMD="pigz -1"      ; REMOTE_DECOMP_CMD="pigz -d"
        elif echo "$BARMAN_TOOLS" | grep -qx 'gzip' && echo "$REMOTE_TOOLS" | grep -qx 'gunzip'; then
    ...
    # --- Inspect remote target path ---
    echo -e "${YELLOW}[7.2/4] Inspecting remote target directory...${NC}"
    REMOTE_TARGET_INFO=$(ssh $SSH_FAST_OPTS \
        "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}" \
        "test -d '$REMOTE_PITR_PATH' && echo EXISTS && ls -la '$REMOTE_PITR_PATH' 2>/dev/null | head -8 && du -sh '$REMOTE_PITR_PATH' 2>/dev/null || echo MISSING" 2>/dev/null || echo "PROBE_FAILED")
    if echo "$REMOTE_TARGET_INFO" | grep -q "^MISSING$"; then
        echo -e "${CYAN}  Remote path does not exist — will be created.${NC}"
    elif echo "$REMOTE_TARGET_INFO" | grep -q "^PROBE_FAILED$"; then
        echo -e "${YELLOW}  Could not probe remote. Aborting.${NC}" >&2
        exit 1
    else
        echo -e "${YELLOW}  Remote path EXISTS — its contents will be wiped before unpack.${NC}"
        echo "$REMOTE_TARGET_INFO" | sed 's/^/    /'
    fi
    echo ""
    ...
    HAS_PV=false ; command -v pv >/dev/null 2>&1 && HAS_PV=true
    ...
    echo -e "  ${CYAN}Compressor (in container):${NC}  ${COMP_NAME}$([ -n "$CONT_COMP_CMD" ] && echo " ($CONT_COMP_CMD)")"
    echo -e "  ${CYAN}Sparse-aware tar:${NC}    $([ -n "$BARMAN_TAR_FLAGS" ] && echo 'yes' || echo 'no')"
    echo -e "  ${CYAN}Phase 1 progress:${NC}    $([ "$HAS_PV" = true ] && echo 'pv (uncompressed bytes)' || echo 'silent (install pv)')"
    echo -e "  ${CYAN}Phase 2 transport:${NC}   rsync --partial --append-verify --info=progress2 (resumable)"
    echo -e "  ${CYAN}Phase 3:${NC}             ssh remote \"${REMOTE_DECOMP_CMD:+$REMOTE_DECOMP_CMD | }tar xpf - -C ${REMOTE_PITR_PATH}\""
    echo -e "  ${CYAN}Cipher:${NC}              aes128-gcm,chacha20-poly1305 (AES-NI fast / fallback)"
    echo -e "  ${RED}${BOLD}Warning:${NC} ${YELLOW}Remote path will be rm -rf'd before unpack.${NC}"
    echo ""
    echo -ne "${BOLD}Action — [${GREEN}s${NC}${BOLD}]hip  [${RED}c${NC}${BOLD}]ancel: ${NC}"
    ...
    INNER_CMD="tar cpf - $BARMAN_TAR_FLAGS -C '$CONTAINER_RECOVERY_DIR' ."
    [ -n "$CONT_COMP_CMD" ] && INNER_CMD="$INNER_CMD | $CONT_COMP_CMD"

    # pv on the host (after docker stdio), shows the actual stream size landing on disk
    set +e
    ...
    P1_EXITS=("${PIPESTATUS[@]}")
    set -e
    P1_FAILED=false ; for ec in "${P1_EXITS[@]}"; do [ "$ec" -ne 0 ] && P1_FAILED=true; done
    if [ "$P1_FAILED" = true ]; then
        echo -e "${RED}✗ Phase 1 failed. PIPESTATUS=[${P1_EXITS[*]}]${NC}" >&2
        echo -e "${YELLOW}  Partial archive at ${HOST_ARCHIVE} kept for inspection.${NC}" >&2
        exit 1
    fi
    PHASE1_ELAPSED=$(( $(date +%s) - PHASE1_START ))
    HOST_ARCHIVE_SIZE=$(du -h "$HOST_ARCHIVE" 2>/dev/null | awk '{print $1}')
    HOST_ARCHIVE_BYTES=$(du -b "$HOST_ARCHIVE" 2>/dev/null | awk '{print $1}')
    echo -e "${GREEN}  ✓ Phase 1 done in ${PHASE1_ELAPSED}s — archive: ${HOST_ARCHIVE_SIZE} (${HOST_ARCHIVE_BYTES} bytes)${NC}"
    if [ -n "$SRC_BYTES" ] && [ -n "$HOST_ARCHIVE_BYTES" ]; then
        RATIO=$(awk "BEGIN{ printf \"%.2f\", $SRC_BYTES / $HOST_ARCHIVE_BYTES }")
        echo -e "${CYAN}  Compression ratio: ${RATIO}× (source ${SRC_BYTES} → archive ${HOST_ARCHIVE_BYTES})${NC}"
    fi
    echo ""

    ##############################################################################
    # PHASE 2 — rsync host → remote with --partial + --append-verify (resumable).
    # If the wire drops at 14 GB, the next rsync picks up the tail.
    ##############################################################################
    echo -e "${YELLOW}[7.3b/4] Phase 2: rsync ${HOST_ARCHIVE} → remote:${REMOTE_ARCHIVE}${NC}"
    PHASE2_START=$(date +%s)
    set +e
    rsync --partial --partial-dir=".rsync-partial" --append-verify --inplace --info=progress2 \
        -e "ssh $SSH_FAST_OPTS" \
        "$HOST_ARCHIVE" \
        "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_ARCHIVE}"
    P2_EC=$?
    set -e
    if [ "$P2_EC" -ne 0 ]; then
        echo -e "${RED}✗ Phase 2 failed (rsync exit $P2_EC).${NC}" >&2
        echo -e "${YELLOW}  Re-run this command to RESUME from the partial — rsync --partial keeps the tail.${NC}" >&2
        exit 1
    fi
    PHASE2_ELAPSED=$(( $(date +%s) - PHASE2_START ))
    echo -e "${GREEN}  ✓ Phase 2 done in ${PHASE2_ELAPSED}s${NC}"
    echo ""

    ##############################################################################
    # PHASE 3 — On remote: wipe target, decompress + unpack archive into final path.
    ##############################################################################
    echo -e "${YELLOW}[7.3c/4] Phase 3: Extract on remote → ${REMOTE_PITR_PATH}${NC}"
    PHASE3_START=$(date +%s)
    if [ -n "$REMOTE_DECOMP_CMD" ]; then
        REMOTE_EXTRACT="rm -rf '${REMOTE_PITR_PATH}' && mkdir -p '${REMOTE_PITR_PATH}' && ${REMOTE_DECOMP_CMD} < '${REMOTE_ARCHIVE}' | tar xpf - -C '${REMOTE_PITR_PATH}'"
    else
        REMOTE_EXTRACT="rm -rf '${REMOTE_PITR_PATH}' && mkdir -p '${REMOTE_PITR_PATH}' && tar xpf - -C '${REMOTE_PITR_PATH}' < '${REMOTE_ARCHIVE}'"
    fi
    if ! ssh $SSH_FAST_OPTS "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}" "$REMOTE_EXTRACT"; then
        echo -e "${RED}✗ Phase 3 (remote extract) failed.${NC}" >&2
        echo -e "${YELLOW}  Archive still at ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_ARCHIVE} — extract manually:${NC}" >&2
        echo -e "${YELLOW}  ssh ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST} \"${REMOTE_EXTRACT}\"${NC}" >&2
        exit 1
    fi
    PHASE3_ELAPSED=$(( $(date +%s) - PHASE3_START ))
    echo -e "${GREEN}  ✓ Phase 3 done in ${PHASE3_ELAPSED}s${NC}"
    echo ""

    # --- Verification ---
    REMOTE_BYTES=$(ssh $SSH_FAST_OPTS \
        "${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}" \
        "du -sb '$REMOTE_PITR_PATH' 2>/dev/null | awk '{print \$1}'" 2>/dev/null | tr -d ' \n')
    echo -e "${YELLOW}[7.4/4] Verification:${NC} source=${SRC_BYTES:-?} bytes  remote-extracted=${REMOTE_BYTES:-?} bytes"
    if [ -n "$SRC_BYTES" ] && [ -n "$REMOTE_BYTES" ] && [ "$SRC_BYTES" = "$REMOTE_BYTES" ]; then
        echo -e "${GREEN}  ✓ Sizes match${NC}"
    else
        echo -e "${YELLOW}  ⚠ Sizes differ (can happen with sparse files / FS differences — review remote ls -la)${NC}"
    fi
    SHIP_TOTAL=$(( PHASE1_ELAPSED + PHASE2_ELAPSED + PHASE3_ELAPSED ))
    echo ""
    echo -e "${CYAN}Total ship time:${NC} ${SHIP_TOTAL}s  (Phase 1: ${PHASE1_ELAPSED}s, Phase 2: ${PHASE2_ELAPSED}s, Phase 3: ${PHASE3_ELAPSED}s)"
    echo ""

    # --- Auto-cleanup on success ---
    # Per user choice: clean both archives AND the container recovery dir on success.
    # The data is safely on the remote — staging copies are no longer needed.
    # If you want to ship to another remote, just re-run perform_pitr.sh.
    echo -e "${YELLOW}Cleaning up staging (host + remote + container) — success path...${NC}"
    rm -f "$HOST_ARCHIVE" 2>/dev/null && echo -e "${GREEN}  ✓ removed ${HOST_ARCHIVE}${NC}"
    ...
    echo -e "${BLUE}${BOLD}========================================${NC}"
    echo -e "${BLUE}${BOLD}  Remote PITR — NEXT STEPS (run on ${REMOTE_PITR_HOST})${NC}"
    echo -e "${BLUE}${BOLD}========================================${NC}"
    echo ""
    echo -e "${CYAN}The recovered data is now at:${NC}"
    echo -e "  ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_PITR_PATH}"
    echo ""
    echo -e "${CYAN}The directory already contains:${NC}"
    echo -e "  - A complete pg_data tree at the requested PITR target time"
    echo -e "  - postgresql.auto.conf with restore_command pre-configured"
    echo -e "  - recovery.signal (so PostgreSQL starts in recovery mode)"
    echo ""
    echo -e "${BOLD}${YELLOW}On the remote host, run (as root or appropriate user):${NC}"
    echo ""
    cat <<EOF
  # 1) Stop PostgreSQL on the remote (use whatever your service manager is)
  sudo systemctl stop postgresql
  # or:  sudo pg_ctlcluster <ver> <name> stop
  # or:  docker stop <container>

  # 2) Back up the current data dir before swapping it (CRITICAL — keeps a rollback)
  sudo mv ${REMOTE_PITR_PATH} ${REMOTE_PITR_PATH}.pre-pitr-\$(date +%Y%m%d_%H%M%S)
  sudo mv <where_you_rsynced_to> ${REMOTE_PITR_PATH}    # only if you rsynced to a staging path
  # If you rsynced directly into ${REMOTE_PITR_PATH}, the data is already in place.

  # 3) Fix ownership (PostgreSQL refuses to start on data dir not owned by postgres)
  sudo chown -R postgres:postgres ${REMOTE_PITR_PATH}
  sudo chmod 700 ${REMOTE_PITR_PATH}

  # 4) Verify recovery config is present
  sudo -u postgres cat ${REMOTE_PITR_PATH}/postgresql.auto.conf | grep -E 'restore_command|recovery_target'
  sudo -u postgres ls -la ${REMOTE_PITR_PATH}/recovery.signal

  # 5) Start PostgreSQL — it will replay WAL until the target time, then promote.
  ...
  # 7) Once promoted (pg_is_in_recovery() returns false), recovery.signal is removed
  #    by PG itself. Verify with:
  sudo -u postgres psql -c "SELECT pg_is_in_recovery(), pg_last_wal_replay_lsn();"
EOF
    echo ""
    echo -e "${CYAN}All staging copies cleaned. Final $PGDATA is on remote at:${NC}"
    echo -e "  ${REMOTE_PITR_USER}@${REMOTE_PITR_HOST}:${REMOTE_PITR_PATH}"
    echo ""
    exit 0
fi
...
if [ "$AUTO_APPLY" = "true" ]; then
    ...
    #   Ciphers       pin AES-NI optimal cipher (~2 GB/s/core); chacha as ARM fallback
    #   Compression=no   disable SSH's zlib
    # Compression is OFF BY DEFAULT for intra-host transfers — gzip caps at
    # ~250 MB/s/core, slower than the cipher itself, so it becomes the new
    # bottleneck. Set INTRA_COMPRESS=1 if the Docker bridge is somehow slow
    # (e.g. overlay network across hosts).
    # SSH options that are intentionally permissive here (container-to-container
    # on the Docker bridge network only — never crosses the host boundary):
    #   StrictHostKeyChecking=no, UserKnownHostsFile=/dev/null
    INTRA_SSH_OPTS="-p 22 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o Compression=no -o Ciphers=aes128-gcm@openssh.com,chacha20-poly1305@openssh.com"
    [ -n "$BARMAN_SSH_KEY" ] && INTRA_SSH_OPTS="-i ${BARMAN_SSH_KEY} ${INTRA_SSH_OPTS}"
    INTRA_RSYNC_OPTS="-a --delete --whole-file --inplace --progress"
